...
public interface IItem {
	...
	public static class Tier {
		...
		// cannot be changed.
		public static Tier WOOD = new Tier().level(0).durability(59).speed(2.0F).damage(0.0F).enchantabilty(15)
				.repairIngredient(planks);
		// TODO(2021-07-14 jcollard): Cobbled Deepslate / Blackstone are listed as
		// repair ingredients
		public static Tier STONE = new Tier().level(1).durability(131).speed(4.0F).damage(1.0F).enchantabilty(5)
				.repairIngredient(VanillaItem.COBBLESTONE);
		public static Tier IRON = new Tier().level(2).durability(250).speed(6.0F).damage(2.0F).enchantabilty(14)
				.repairIngredient(VanillaItem.IRON_INGOT);
		public static Tier DIAMOND = new Tier().level(3).durability(1561).speed(8.0F).damage(3.0F).enchantabilty(10)
				.repairIngredient(VanillaItem.DIAMOND);
		public static Tier GOLD = new Tier().level(0).durability(2031).speed(9.0F).damage(4.0F).enchantabilty(15)
				.repairIngredient(VanillaItem.GOLD_INGOT);
		public static Tier NETHERITE = new Tier().level(4).durability(32).speed(12.0F).damage(0.0F).enchantabilty(22)
	...
	public static class Adapter<T extends Item> {
		...
		public Adapter(@Nonnull IItem item, Builder<T> builder) {
			...
			Class<? extends T> dynamicType = new ByteBuddy().subclass(builder.klass)
					.method(ElementMatchers.named("finishUsingItem")).intercept(implementation)
					.method(ElementMatchers.named("getUseAnimation")).intercept(implementation)
					.method(ElementMatchers.named("getUseDuration")).intercept(implementation)
					.method(ElementMatchers.named("use")).intercept(implementation).make()
