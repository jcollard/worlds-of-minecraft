...
public class Utils {
    ...
    public String getModsToML(@Nonnull IMinecraftMod mod) {
        ...
        if (mod.getLogo() != null) {
            mapping.put("logoFile", mod.getLogo()
                                       .getFileName());
        }
        ...
        b.append("[[mods]]\n");
        this.appendKeys(b, mapping, "modId", "version", "displayName", "updateJSONURL", "displayURL", "logoFile",
                "credits", "authors");
    ...
    public Path getAssetsDir(@Nonnull IMinecraftMod mod) {
        Preconditions.checkArgument(mod != null);
        return Paths.get(rootDir)
                    .resolve("src/main/resources/assets/" + mod.getModId());
    }
    ...
    /**
     * Converts a string to a registry safe name by replacing all non-safe
    ...
    public String validateName(String name) {
        if (!name.matches("^[A-Za-z0-9]*[A-Za-z0-9 ][A-Za-z0-9 ]*$")) {
            throw new InvalidNameException(
                    "The name \"" + name + "\" is invalid. A name may only contain letters, numbers, and spaces.");
        }
        return name;
    }
    ...
    public String validateRegistryName(String name) {
        if (!name.matches("^[a-z][a-z0-9_]*$")) {
            throw new InvalidNameException("The registry name \"" + name
                    + "\" is invalid. A registry name must begin with a letter and contain only lowercase letters, numbers, and underscores.");
        }
        return name;
    }
    ...
    public String validateModId(String modId) {
        if (!modId.matches("^[a-z][a-z_]*$")) {
            throw new InvalidNameException("The modId \"" + modId
                    + "\" is invalid. A modId must begin with a letter and contain only lowercase letters and underscores.");
        }
        return modId;
    }
